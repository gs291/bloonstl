...
export default class MonkeysAPI extends DataSource {
    ...
    async getMonkeyById({ id }) {
        try {
            ...
            if (!monkey.length) { throw `Could not find monkey with name: ${id}`; }
            return monkey[0];
        } catch (e) {
            console.log(` -- getMonkeyById DataSource Error: ${e}`);
            return [];
        }
    }
    ...
    async getMonkeyByName({ name }) {
        try {
            ...
            if (!monkey.length) { throw `Could not find monkey with name: ${name}`; }
            return monkey[0];
        } catch (e) {
    ...
    async getMonkeysByType({ type }) {
        try {
            ...
            if (!monkeys.length) { throw `Could not find monkeys with type: ${type}`; }
            return monkeys;
        } catch (e) {
