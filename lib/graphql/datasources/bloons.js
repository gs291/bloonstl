...
export default class BloonsAPI extends DataSource {
    ...
    /**
     * Parses a given boss from the database and returns
     *   cleanly json formatted object. It includes a
     *   type. (0 for regular, 1 for elite, 2 for special)
     *
     * @return {Object} Cleanly parsed boss
     */
    parseBosses(boss) {
     ...
     * @return {Object} Cleanly parsed boss
     */
    parseBossSpecial(bossSpecial) {
        return {
            name: bossSpecial.name.split('|'),
            hp: bossSpecial.hp.split(';'),
            rbe: bossSpecial.rbe.split(';'),
        }
    }
    ...
    async getBloonByVarName({varName}) {
        ...
        if (!bloon.length) {
            ...
            if (!boss.length) {
                ...
                bossParsed.special = this.parseBossSpecial(bossSpecial[0]);
                bossParsed.elite = this.parseBosses(bossElite[0]);
                bossParsed.elite.special = this.parseBossSpecial(bossEliteSpecial[0]);
