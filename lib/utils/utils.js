...
const parseModifier = (modifier, tiers, stats, level) => {
    ...
    if(parsed) {
        stats.targets.push({"isPro": parsed[1] === "+", "code": parsed[2]});
        return;
    }
    ...
    const parseRecursive = (statsExtra, name, modifiers) => {
        if (statsExtra[name]) {
            parseAbilityModifiers(modifiers, tiers, statsExtra[name], level + 1);
        } else {
            const tempStats = getInitialTowerStats();
            parseAbilityModifiers(modifiers, tiers, tempStats, level + 1);
