...
/**
 * Get the enemy name from the varName
 */
export const getEnemyName = {
    "red": "Red Bloon",
    "blue": "Blue Bloon",
    "grn": "Green Bloon",
    "ylw": "Yellow Bloon",
 ...
 * Converts a stat attribute string into a readable string
 *
 * @param {string} special the special move from the boss
 *
 * @return {Array} Converted stat attribute text
 */
export const parseBossSpecial = (special) =>
    special.split(',').map(value => {
        const bloonItem = value.split('-');
        if (bloonItem.length === 2) {
            return `${getEnemyName[bloonItem[0]]} (x${bloonItem[1]})`;
        }
