...
export const parseAbilityModifiers = (modifiers, tiers, stats, level = 1) => {
    const split = level === 1
        ? ','
        : level === 2 ? ';' : '=';
    ...
    for (i; i < modifiers.length; i++) {
        parseModifier(modifiers[i], tiers, stats, level)
    }
}


const parseModifier = (modifier, tiers, stats, level) => {
    // Matches for math usages (e.g. '-4.50', or '+1')
    let parsed = modifier.match(/^([\+\-\*])?([0-9]+\.?[0-9]*|\.[0-9]+)(%)?(\w*)/);
    if (parsed) {
    ...
    if(parsed) {
        // console.log(`(${level}):` ,"+- BLOON/BLIMP:", parsed);
        stats.targets.push(parsed[0]);
        return;
    }

    parsed = modifier.match(/^abil:(\w*):(.*)/);
    if(parsed) {
        // console.log(`(${level}):` ,"abil:", parsed);

        if (stats.abils[parsed[1]]) {
            parseAbilityModifiers(parsed[2], tiers, stats.abils[parsed[1]], level + 1);
        } else {
            const tempStats = getInitialTowerStats();
            parseAbilityModifiers(parsed[2], tiers, tempStats, level + 1);

            stats.abils[parsed[1]] = tempStats;
        }
        return;
    }
    ...
    if(parsed) {
        // console.log(`(${level}):` ,"status:", parsed);

        if (stats.statuses[parsed[1]]) {
            parseAbilityModifiers(parsed[2], tiers, stats.statuses[parsed[1]], level + 1);
        } else {
            const tempStats = getInitialTowerStats();
            parseAbilityModifiers(parsed[2], tiers, tempStats, level + 1);

            stats.statuses[parsed[1]] = tempStats;
        }
        return;
    }
    ...
    if(parsed) {
        // console.log(`(${level}):` ,"attack:", parsed);

        if (stats.attacks[parsed[1]]) {
            parseAbilityModifiers(parsed[2], tiers, stats.attacks[parsed[1]], level + 1);
        } else {
            const tempStats = getInitialTowerStats();
            parseAbilityModifiers(parsed[2], tiers, tempStats, level + 1);

            stats.attacks[parsed[1]] = tempStats;
        }
        return;
    }
    ...
    if(parsed) {
        // console.log(`(${level}):` ,"buff:", parsed);

        if (stats.buffs[parsed[1]]) {
            parseAbilityModifiers(parsed[2], tiers, stats.buffs[parsed[1]], level + 1);
        } else {
            const tempStats = getInitialTowerStats();
            parseAbilityModifiers(parsed[2], tiers, tempStats, level + 1);

            stats.buffs[parsed[1]] = tempStats;
        }
        return;
    }
    ...
    if(parsed) {
        if (parsed[1]) {
            stats.modifiers.damage_type = parsed[1];
        }
        return;
    }
    ...
    if(parsed) {
        if (parsed[1] && parsed[2] && parsed[3]) {
            if (tiers.top_path >= parseInt(parsed[1]) && tiers.middle_path >= parseInt(parsed[2]) && tiers.bottom_path >= parseInt(parsed[3])) {
                if (parsed[4]) {
                    console.log("ADDING CROSS", `(${level}):` ,"cross:", parsed);
