...
const parseModifier = (modifier, tiers, stats, parseTooltip, level) => {
    ...
    if(parsed && parsed[1] && parsed[2] && parsed[3]) {
        ...
        if ((tiers.top_path >= parseInt(parsed[1]) && tiers.top_path <= 5)
            && (tiers.middle_path >= parseInt(parsed[2]) && tiers.middle_path <= 5)
            && (tiers.bottom_path >= parseInt(parsed[3]) && tiers.bottom_path <= 5)) {
            if (parsed[4]) {
