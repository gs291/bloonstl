...
export const getTierColor = (tier) => {
    switch(tier){
        ...
        default:  return siteColors.tier.b.color;
    }
}
...
export const parseForm = (form) => {
    ...
    if (form["slider-top"].value > 0 || form["slider-middle"].value > 0 || form["slider-bottom"].value > 0) {
        if (form["ability-tier"].value === "") {
            return {"err": "Ability path selected yet ability tier is not"};

        }
    }

    if ((pros.length || cons.length) && form["ability-tier"].value === "") {
        return {"err": "Ability pros/cons selected yet ability tier is not"};
    }

    body.tower = form.tower.value
    body["tower-tier"] = form["tower-tier"].value;

    if (form["ability-tier"].value) {
        body["ability-tier"] = form["ability-tier"].value
        body["ability-top"] =  form["slider-top"].value;
        body["ability-middle"] =  form["slider-middle"].value;
        body["ability-bottom"] =  form["slider-bottom"].value;
