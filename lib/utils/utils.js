 ...
 * @param {string} [tier] The tier for the monkey
 ...
 */
export const getMonkeyColor = (type, theme, tier, card = false, hover = false) => {
    if (tier){
        return getTierCardColor(tier, theme, hover);
    }
 ...
 * @param {Object} theme MUI 5 theme for the project
 * @param {string} [tier] The tier for the hero
 ...
 */
export const getHeroColor = (heroName, theme, tier, card = false, hover = false) => {
    if (tier){
        return getTierCardColor(tier, theme, hover);
    } else if(card) {
        return hover ? theme.palette.tower.heroes.hover : theme.palette.tower.heroes.card;
    }
...
/**
 * Get the tier card color
 *
 * @param {string} tier The tier
 * @param {Object} theme MUI 5 theme for the project
 * @param {boolean} [hover=false] Shows if the tier card hover color should be returned
 *
 * @return {string} Tier card HEX color code
 */
export const getTierCardColor = (tier, theme, hover) => {
    switch(tier){
        case "s": return hover ? theme.palette.tier.s.hover : theme.palette.tier.s.card;
        case "a": return hover ? theme.palette.tier.a.hover : theme.palette.tier.a.card;
        case "b": return hover ? theme.palette.tier.b.hover : theme.palette.tier.b.card;
        default: return hover ? theme.palette.tier.c.hover : theme.palette.tier.c.card;
    }
}
 ...
 * @param {string} tier The tier
 * @param {Object} theme MUI 5 theme for the project
 * @param {boolean} [selected] Shows if the tier is selected or not
 *
 * @return {string} Tier HEX color code
 */
export const getTierColor = (tier, theme, selected) => {
    switch(tier){
        case "s": return selected ? theme.palette.tier.s.selected : theme.palette.tier.s.color;
        case "a": return selected ? theme.palette.tier.a.selected : theme.palette.tier.a.color;
        case "b":  return selected ? theme.palette.tier.b.selected : theme.palette.tier.b.color;
        default:  return selected ? theme.palette.tier.c.selected : theme.palette.tier.c.color;
    }
}
